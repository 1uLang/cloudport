...
$assist.init = function (cb_stack) {
    ...
    $.ajax({
        ...
        error: function () {
            ...
            console.error('无法连接到cloudfort助手，可能其尚未运行！');
        }
    });

    cb_stack.exec();
};
//<li>堡垒机助手windows版本：<a href="/static/download/teleport-assist-macos-3.2.2.dmg"><i class="fa fa-download fa-fw"></i> cloudfort-assist-windows-3.2.2.exe</a> </li>
//<li>堡垒机助手macos版本：<a href="/static/download/teleport-assist-windows-3.2.2.exe"><i class="fa fa-download fa-fw"></i>cloudfort-assist-macos-3.2.2.dmg</a> </li>
$assist.alert_assist_not_found = function () {
    console.log($assist.errcode);
    if($assist.errcode === TPE_NO_ASSIST) {
        $assist.dom.msg_box_title.html('未检测到CLOUDFORT助手');
        $assist.dom.msg_box_info.html('需要CLOUDFORT助手来辅助远程连接，请确认本机运行了CLOUDFORT助手！');
        $assist.dom.msg_box_desc.html('如果您尚未运行CLOUDFORT助手，请 <a href="/static/download/cloudfort-assist-windows-3.2.2.exe"><strong>下载最新windows版本堡垒机助手安装包</strong></a>或<a href="/static/download/cloudfort-assist-macos-3.2.2.exe"><strong>下载最新macos版本堡垒机助手安装包</strong></a> 并安装。一旦运行了CLOUDFORT助手，即可重新进行远程连接。');
    } else if($assist.errcode === TPE_OLD_ASSIST) {
        $assist.dom.msg_box_title.html('CLOUDFORT助手需要升级');
        $assist.dom.msg_box_info.html('检测到CLOUDFORT助手版本 v'+ $assist.version +'，但需要最低版本 v'+ $assist.ver_require+'。');
        $assist.dom.msg_box_desc.html('请 <a href="/static/download/cloudfort-assist-windows-3.2.2.exe"><strong>下载最新windows版本堡垒机助手安装包</strong></a>或<a href="/static/download/cloudfort-assist-macos-3.2.2.exe"><strong>下载最新macos版本堡垒机助手安装包</strong></a> 并安装。一旦升级了CLOUDFORT助手，即可重新进行远程连接。');
    }
            ...
            if (ret.code === TPE_OK) {
				...
				if ($app.options.url_proto){
                    ...
                    $.ajax({
                        ...
                        error: function () {
                            func_error(TPE_NO_ASSIST, '无法连接到cloudfort助手，可能尚未启动！');
                        }
                    });
                }
            } else {
                if (ret.code === TPE_NO_CORE_SERVER) {
                    func_error(ret.code, '远程连接请求失败，可能cloudfort核心服务尚未启动！' + ret.message);
                } else {
                    func_error(ret.code, ret.message);
                }
            }
